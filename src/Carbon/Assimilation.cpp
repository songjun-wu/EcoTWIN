...
int Basin::Assimilation(Control &ctrl, Atmosphere &atm, Param &par){
  ...
  for (unsigned int j = 0; j < _sortedGrid.row.size(); j++) {
    ...
    double LAI_sun = min(LAI_total, (1.0 - exp(- 0.5 * LAI_total)));  
    double LAI_shade = LAI_total - LAI_sun;

    // === Calculate shaded and sunlit leaf PAR per leaf area ===
    // Diffuse and direct PAR per ground area from total PAR
    double PPFD_diffuse = PAR_mol * ratio_diffuse; // Diffuse PAR in mol photons * (m-2 ground area) * (s-1)
    double PPFD_direct = PAR_mol - PPFD_diffuse; // Direct PAR in mol photons * (m-2 ground area) * (s-1)
    // Shaded leaf PAR
    ...
    double PAR_shade = PPFD_diffuse * ( 1 - exp(-0.5 * LAI_total)) / LAI_total;  // Diffuse PAR in mol photons * (m-2 leaf area) * (s-1)
    // Sunlit leaf PAR
    ...
    double PAR_sun = (PPFD_direct * 1) + PAR_shade;  // Direct PAR in mol photons * (m-2 leaf area) * (s-1)
    ...
    VC_max = carboxylation_rate * exp(EV * energy_scaling_factor);
    ...
    Photosynthesis_C3(j, ctrl.Simul_tstep, PAR_sun, LAI_sun, Ta_k, TC_c, air_pressure, energy_scaling_factor, co2_mol, co2_leaf_mol, 
      KC, KO, VC_max, Jmax, gamma, water_limitation_factor, carboxylation_rate, *_NPP, *_canopy_conductance);
    
    Photosynthesis_C3(j, ctrl.Simul_tstep, PAR_shade, LAI_shade, Ta_k, TC_c, air_pressure, energy_scaling_factor, co2_mol, co2_leaf_mol, 
      KC, KO, VC_max, Jmax, gamma, water_limitation_factor, carboxylation_rate, *_NPP, *_canopy_conductance);
...
int Basin::Photosynthesis_C3(int j, int timestep, double PAR_mol, double LAI, double Ta_k, double TC_c, double air_pressure, double energy_scaling_factor, double co2_mol, double co2_leaf_mol,
    ...
    double G0, B, C, J1, W1;  // Water stressed stomatal conductance [m s-1 leaf area] and related parameters
    double canopy_conductance_leaf;  // Canopy stomatal conductance per leaf area  [m s-1 leaf area]
    double conductance_d;  // Related parameters
    ...
    dark_respiration = FRDC3 * carboxylation_rate * \
                        exp(ER * energy_scaling_factor) * dark_hib * hitin_hib;
    ...
    // Calculate canopy conductance of H2O without water stress
    canopy_conductance_leaf = max(0.0, 1.6 * (GPP - dark_respiration) / (co2_mol - co2_leaf_mol) * R_gas * Ta_k / air_pressure);
    canopy_conductance_leaf *= water_limitation_factor; // Canopy conductance with water stress

    // Calculate Carboxylilation controlled assimilation JE under water stress
    // Known: A = JE - RD
    // Known: A = canopy_conductance_leaf / 1.6 * (CA - Ci) * pressure / R / t_air 
    // => JE - RD = canopy_conductance_leaf / 1.6 * (CA - Ci) * pressure / R / t_air
    // after transformation we get: Ci = CA - (JE - RD) / (canopy_conductance_leaf / 1.6 * pressure / R / t_air)
    // Here we esemble denominator as G0 (eq1): G0 = canopy_conductance_leaf / 1.6 * pressure / R / t_air AND
    ...
    // eq2
    G0 = canopy_conductance_leaf / 1.6 * air_pressure / R_gas / Ta_k;
    // eq4
    ...
    // Set Ci in eqtion : JC = VC_max * (Ci - Gam) / (Ci + KC * (1 + OX/KO))
    ...
    sv_canopy_conductance.val[j] += canopy_conductance_leaf * LAI;  // Canopy conductance in m s-1
