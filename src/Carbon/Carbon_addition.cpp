...
int Basin::Carbon_addition(Control &ctrl, Param &par){
    ...
    for (unsigned int j = 0; j < _sortedGrid.row.size(); j++) {
      ...
      leaf_shedding = max(_LAI->val[j] * par._LAI_shed_ceof->val[j], _LAI_old->val[j] - _LAI->val[j]);  // Normal shedding or due to plant mortality
      ...
      // TODO: HARVEST?
      // TODO: Herbivory loss from plant_green_CP & green_NP? No considered at the moment
      ...
      if (ctrl.opt_nitrogen_sim){
        potential_N_for_plant_growth = (NPP_2_green + NPP_2_root_exudates)*par._NC_ratio_plant_green->val[j] + NPP_2_wood*par._NC_ratio_plant_wood->val[j];
    ...
    NPP_2_root_exudates *= fct_N_limitation;
    ...
    if (ctrl.opt_nitrogen_sim){
      ...
      double soluble_NP1 = _don_layer1->val[j] * _theta1->val[j] * depth1;
      double soluble_NP2 = _don_layer2->val[j] * _theta2->val[j] * depth2;
      double soluble_NP3 = _don_layer3->val[j] * _theta3->val[j] * depth3;
      ...
      _fast_NP1_nonwood->val[j] += (C_green_2_litter_nonwood+NPP_2_root_exudates) * (1 - par._frac_litter_to_soluble_nonwood->val[j]) * par._NC_ratio_fast_pool_nonwood->val[j];  // Only green pool contains ntrogen (reserve pool is nitrogen free)
      soluble_NP1 += (C_green_2_litter_nonwood+NPP_2_root_exudates) * par._frac_litter_to_soluble_nonwood->val[j] * par._NC_ratio_fast_pool_nonwood->val[j];
      soluble_DIN_NP1 += (C_green_2_litter_nonwood+NPP_2_root_exudates) * (par._NC_ratio_plant_green->val[j] - par._NC_ratio_fast_pool_nonwood->val[j]);
      // Wood pools in the first layer
      soluble_NP1 += C_wood_2_litter_wood * par._frac_litter_to_soluble_wood->val[j] * par._NC_ratio_fast_pool_wood->val[j];
      soluble_DIN_NP1 += C_wood_2_litter_wood * (par._NC_ratio_plant_wood->val[j] - par._NC_ratio_fast_pool_wood->val[j]);

      // Wood pools in the second layer
      soluble_NP2 += C_wood_2_litter_wood * par._frac_litter_to_soluble_wood->val[j] * par._NC_ratio_fast_pool_wood->val[j];
      soluble_DIN_NP2 += C_wood_2_litter_wood * (par._NC_ratio_plant_wood->val[j] - par._NC_ratio_fast_pool_wood->val[j]);

      // Wood pools in the third layer
      soluble_NP3 += C_wood_2_litter_wood * par._frac_litter_to_soluble_wood->val[j] * par._NC_ratio_fast_pool_wood->val[j];
      soluble_DIN_NP3 += C_wood_2_litter_wood * (par._NC_ratio_plant_wood->val[j] - par._NC_ratio_fast_pool_wood->val[j]);

      // Update DON and DIN concentrations
      _don_layer1->val[j] = soluble_NP1 / (_theta1->val[j] * depth1);
      _don_layer2->val[j] = soluble_NP2 / (_theta2->val[j] * depth2);
      _don_layer3->val[j] = soluble_NP3 / (_theta3->val[j] * depth3);
