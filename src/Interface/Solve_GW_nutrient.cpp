...
int Basin::Solve_GW_nutrient(Control &ctrl, Atmosphere &atm){
    /*
    ### GW:
        - irrigation_from_GW
        (_GW_old)
    ...
    if (ctrl.opt_carbon_sim==1){
      if (ctrl.opt_baseflow_mixing == 0){
        for (unsigned int j = 0; j < _sortedGrid.row.size(); j++) {
            Mixing_full(_GW_old->val[j], _doc_GW->val[j], _Perc_vadose->val[j], _doc_vadose->val[j]);
        }
      }
    }
    // Nitrogen
