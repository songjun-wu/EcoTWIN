...
int Basin::Solve_channel_nutrient(Control &ctrl, Atmosphere &atm, Param &par){
            ...
            if (_chnlength->val[j] > 0){    // If this is a channel cell 
                if (_Echan->val[j] > roundoffERR){
                    if (_chanS->val[j] > roundoffERR){
                        _doc_chanS->val[j] *= (_chanS->val[j] + _Echan->val[j]) / _chanS->val[j];  // Enrichment due to channel evaporation
                    } else {
                        _doc_chanS->val[j] = 0.0;
                    }   
                }
            }
        }
