...
int Basin::Solve_routing_transport(Control &ctrl, Param &par, \
                                   svector &_sv_conc_pond, svector &_sv_conc_layer1, svector &_sv_conc_layer2, svector &_sv_conc_layer3, \
                                   svector &_sv_conc_vadose, svector &_sv_conc_GW, svector &_sv_conc_chanS){
    ...
    */
    ...
    int from_j;
    int lat_ok;
    double sv_conc_in_all_acc, q_in_all;
    ...
    for (unsigned int j = 0; j < _sortedGrid.row.size(); j++) {
        ...
        Mixing_full(_pond_old->val[j], _sv_conc_pond.val[j], _ovf_in->val[j], _flux_ovf_in_acc->val[j] / _ovf_in->val[j]);

        if (lat_ok == 1){  // Add 18O in overland outflow to the overland inflow of downstream cell
            _flux_ovf_in_acc->val[from_j] += _sv_conc_pond.val[j] * _ovf_out->val[j];
        }
        ...
        if (ctrl.opt_reinfil == 1){ 
            ...
            Mixing_full(_theta1_old->val[j] * _depth1->val[j], _sv_conc_layer1.val[j], _rinfilt->val[j], _sv_conc_pond.val[j]);
            // Re-percolation mixing with layer 2
            Mixing_full(_theta2_old->val[j] * _depth2->val[j], _sv_conc_layer2.val[j], _rPerc1->val[j], _sv_conc_layer1.val[j]);
            // Re-percolation mixing with layer 3
            Mixing_full(_theta3_old->val[j] * par._depth3->val[j], _sv_conc_layer3.val[j], rPerc2, _sv_conc_layer2.val[j]);
            // Vadose mixing with repercolation from layer 3
            Mixing_full(_vadose_old->val[j], _sv_conc_vadose.val[j], rPerc3, _sv_conc_layer3.val[j]); 
        ...
        if (ctrl.opt_baseflow_mixing == 0){  // Still use full mixing
            if (_GWf_in->val[j] > roundoffERR) {
                Mixing_full(_GW_old->val[j] + rPerc_vadose, _sv_conc_GW.val[j], _GWf_in->val[j], _flux_GWf_in_acc->val[j] / _GWf_in->val[j]);
            }
            ...
            if (influx > roundoffERR) {
                cinflux = (_flux_GWf_in_acc->val[j] + (_Perc_vadose->val[j]+rPerc_vadose)*_sv_conc_vadose.val[j]) / influx;
                Mixing_baseflow(_GW_old->val[j], _sv_conc_GW.val[j], influx, cinflux, _GWf_out->val[j]);
            }
        }

        if (lat_ok == 1){  // Add 18O in GW outflow to the GW inflow of downstream cell
            _flux_GWf_in_acc->val[from_j] += _sv_conc_GW.val[j] * _GWf_out->val[j];
        }
        ...
        if (_chnwidth->val[j] > roundoffERR){
            ...
            sv_conc_in_all_acc = _flux_Qupstream_acc->val[j] + _sv_conc_pond.val[j] * _ovf_toChn->val[j] + _sv_conc_vadose.val[j] * _interf_toChn->val[j] + _sv_conc_GW.val[j] * _GWf_toChn->val[j]; 
            q_in_all = _Qupstream->val[j] * m3s_to_m + _ovf_toChn->val[j] + _interf_toChn->val[j] + _GWf_toChn->val[j];

            Mixing_full(_chanS_old->val[j], _sv_conc_chanS.val[j], q_in_all, sv_conc_in_all_acc / q_in_all);

            if (lat_ok == 1){  // Add 18O in discharge to inflow of the downstream channel storage
                _flux_Qupstream_acc->val[from_j] += _sv_conc_chanS.val[j] * _Q->val[j] * m3s_to_m;
            }
