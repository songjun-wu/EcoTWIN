...
int Basin::Sort_crop_management(Control &ctrl, Param &par){

  double fraction_crop;  // The proportion of crop in the grid cell [decimal]
  int idx;

  if (par.sort_crop_management_OK == 0){
    ...
    for (int j = 0; j < _sortedGrid.row.size(); j++) {
      ...
      for (int i = 0; i < num_landuse; i++) {
        idx = landuse_idx[i];
        if (is_crop[idx] == 1){
          fraction_crop += par.param_category->val[idx][j];
        }
      }
      _fraction_crop->val[j] = fraction_crop;
    }
    */
