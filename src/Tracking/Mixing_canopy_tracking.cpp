...
int Basin::Mixing_canopy_tracking(Control &ctrl, Atmosphere &atm){
    ...
    */
    ...
    if (ctrl.opt_tracking_isotope==1) {
        ...
        Solve_canopy_transport(atm, *_d18o_I, *atm._d18o_P, *_d18o_pond, false); // enrichment flag = false
    ...
    if (ctrl.opt_tracking_trans_age==1){
        // Mixing canopy storage with precipitation input
        for (unsigned int j = 0; j < _sortedGrid.row.size(); j++) {
            Mixing_full(_I_old->val[j], _trans_age_I->val[j], atm._P->val[j], 0.0);
            _trans_age_pond->val[j] = _trans_age_I->val[j]; // Align the composition in ponding water with that in throughfall
        }
    }
