...
int main(){
  ...
  float advance_management = 0;  // reset to zero when management_input (e.g., drainage depth) is updated
  ...
  while (oControl->current_ts < oControl->Simul_end){
    ...
    advance_management += oControl->Simul_tstep;
    ...
    if (advance_management >= oControl->Management_input_tstep) {
      if (oControl->opt_managementTs_input_format == 1){
        oBasin->read_managementTs(*oControl);
      } else if  (oControl->opt_managementTs_input_format == 2) {
        oBasin->update_managementTs(*oControl, *oParam);
      }
