...
def run_dream(parameters, likelihood, nchains=5, niterations=50000, start=None, restart=False, verbose=True, nverbose=10, tempering=False, mp_context=None, **kwargs):
    ...
    log_ps : list of arrays
        ...
        """
    print(mp.cpu_count(), len(os.sched_getaffinity(0)))
    ...
    try:
        if tempering:
            ...
            if type(start) is list:
                args = zip([step_instance]*nchains, [niterations]*nchains, start, [verbose]*nchains, [nverbose]*nchains, np.arange(nchains), [kwargs['savePath']]*nchains, [kwargs['total_iterations']]*nchains, [kwargs['model_name']]*nchains)

            else:
                args = list(zip([step_instance]*nchains, [niterations]*nchains, [start]*nchains, [verbose]*nchains, [nverbose]*nchains, np.arange(nchains), [kwargs['savePath']]*nchains, [kwargs['total_iterations']]*nchains, [kwargs['model_name']]*nchains))
