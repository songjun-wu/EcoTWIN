...
class Param:
    ...
    # To be calibrated
    ref['carboxylation_rate'] = {'type':'landuse',   'log':0, 'file':'carboxylation_rate',   'min':[], 'max':[], 'fix_value':np.array([100,80,80,50,70,70])*1e-6} # Carboxylation rate at 25 degree celcius [mol(CO2)/(m2*s)]
    # To be calibrated
    ref['ETransport'] = {'type':'landuse',   'log':0, 'file':'ETransport',   'min':[], 'max':[], 'fix_value':np.array([190,145,145,110,130,130])*1e-6} # Maximum electron transport rate at 25 Celsius [1.E-6 * Mol/m^2 leafarea/s] (Jmax=1.9*V_max^25 for C3 plants)
    ...
    ref['plant_wood_CP_max'] = {'type':'landuse',   'log':0, 'file':'plant_wood_CP_max',   'min':[], 'max':[], 'fix_value':np.array([0,0,0,400,50,50])*12} # The maximum carbon content in wood pool [gC/m2] <== [molC/m2]; the global maximum should be ~2000*12 gC/m2, here we set to 400*12 given the lower carbon biomass in Europe
