    ...
    */
    ...
    if (ctrl.opt_tracking_isotope==1) {
        ...
        for (unsigned int j = 0; j < _sortedGrid.row.size(); j++) {
            ...
            Mixing_full(_theta3_old->val[j] * par._depth3->val[j] + rPerc2 - rPerc3, _d18o_layer3->val[j], _interf_in->val[j], _flux_interf_in_acc->val[j] / _interf_in->val[j]);
            ...
            if (lat_ok == 1){  // Add 18O in interflow outflow to the inferflow inflow of downstream cell
                _flux_interf_in_acc->val[from_j] += _d18o_layer3->val[j] * _interf_out->val[j];
            }

            // GW flow mixing with lateral inflow
            Mixing_full(_GW_old->val[j] + rPerc3 + _rrPerc3->val[j], _d18o_GW->val[j], _GWf_in->val[j], _flux_GWf_in_acc->val[j] / _GWf_in->val[j]);
            ...
            if (_chnwidth->val[j] > roundoffERR){
                ...
                d18o_in_all_acc = _flux_Qupstream_acc->val[j] + _d18o_pond->val[j] * _ovf_toChn->val[j] + _d18o_layer3->val[j] * _interf_toChn->val[j] + _d18o_GW->val[j] * _GWf_toChn->val[j]; 
    ...
    if (ctrl.opt_tracking_age==1) {
        ...
        for (unsigned int j = 0; j < _sortedGrid.row.size(); j++) {
            ...
            if (ctrl.opt_reinfil == 1){ 
                ...
                // GW mixing with repercolation from layer 3
                Mixing_full(_GW_old->val[j], _age_GW->val[j], rPerc3, _age_layer3->val[j]);
            }

            
            // Interflow mixing with lateral inflow
            Mixing_full(_theta3_old->val[j] * par._depth3->val[j] + rPerc2 - rPerc3, _age_layer3->val[j], _interf_in->val[j], _flux_interf_in_acc->val[j] / _interf_in->val[j]);
            Mixing_full(_GW_old->val[j] + rPerc3, _age_GW->val[j], _rrPerc3->val[j], _age_layer3->val[j]); // GW mixed with repercolation due to excess interflow
            if (lat_ok == 1){  // Add 18O in interflow outflow to the inferflow inflow of downstream cell
                _flux_interf_in_acc->val[from_j] += _age_layer3->val[j] * _interf_out->val[j];
            }

            // GW flow mixing with lateral inflow
            Mixing_full(_GW_old->val[j] + rPerc3 + _rrPerc3->val[j], _age_GW->val[j], _GWf_in->val[j], _flux_GWf_in_acc->val[j] / _GWf_in->val[j]);
            ...
            if (_chnwidth->val[j] > roundoffERR){
                                ...
                                // Upstream inflow              Overland flow to channel                  Interflow to channel                           GW flow to channel
                age_in_all_acc = _flux_Qupstream_acc->val[j] + _age_pond->val[j] * _ovf_toChn->val[j] + _age_layer3->val[j] * _interf_toChn->val[j] + _age_GW->val[j] * _GWf_toChn->val[j]; 
                q_in_all = _Qupstream->val[j] * m3s_to_m + _ovf_toChn->val[j] + _interf_toChn->val[j] + _GWf_toChn->val[j];
