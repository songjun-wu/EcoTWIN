...
int Basin::Solve_canopy(Control &ctrl, Param &par, Atmosphere &atm) {
    ...
    for (unsigned int j = 0; j < _sortedGrid.row.size(); j++) {
        ...
        Interception_1(par, atm, j);
        } else if (ctrl.opt_intecept == 2){
        Interception_2(par, atm, j);
        }
    }
    return EXIT_SUCCESS;
}
...
int Basin::Interception_1(Param &par, Atmosphere &atm, int j) {
...
int Basin::Interception_2(Param &par, Atmosphere &atm, int j) {
    // Parameters
    double alpha = par._alpha->val[j];
